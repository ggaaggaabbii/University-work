...
int addOffer(Controller *c, Offer* offer) {
	if (addElemet(c->repo, offer)) {
		Operation *op = createOperation(createCopyOffer(offer), offer->destination,
		                                offer->departureDate, "Add", "Delete", "", 0);
		addOperation(c->u, op);
		return 1;
	}
	return 0;
...
int deleteOffer(Controller *c, char* destination, char* departureDate) {
	Offer *x = find(c->repo, destination, departureDate);
	if (x == 0) {
		return 0;
	}
	Operation *op = createOperation(createCopyOffer(x), destination,
	                                departureDate, "Delete", "Add", "", 0);
	addOperation(c->u, op);
	return deleteElemet(c->repo, destination, departureDate);
}
...
int updateOffer(Controller *c, char* destination, char* departureDate, void* newElement) {
	...
	char* a = (char*)malloc(sizeof(char) * (strlen(x->type) + 1));
	strcpy(a, x->type);
	double p = x->price;
	if (updateElement(c->repo, destination, departureDate, newElement)) {
		Operation *op = createOperation(createCopyOffer(newElement), destination,
		                                departureDate, "Update", "Update", a, p);
		addOperation(c->u, op);
		free(a);
		return 1;
	}
