...
class rentalController:
	...
	def mostRentedMovie(self):
		...
		mostRentedByNumberOfRents = sorted(mostRentedByNumberOfRents, key = lambda x : x[1], reverse = True)
		mostRentedByNumberOfDays = sorted(mostRentedByNumberOfDays, key = lambda x : x[1], reverse = True)
	...
	def mostActiveClient(self):
		...
		mostActiveClients = sorted(mostActiveClients, key = lambda x : x[1], reverse = True)
	...
	def lateRentals(self, params):
		...
		lateRents = sorted(lateRents, key = lambda x : x[1], reverse = True)
		return lateRents
