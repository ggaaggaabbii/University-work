...
void apply_bw_filter(Mat &img, int st, int fn, int me) {
  cerr << "> proc " << me << " applying black & white filter\n";
  for (int i = st; i < fn; ++ i) {
    for (int j = 0; j < img.cols; ++ j) {
      Vec3b px = img.at<Vec3b>(i, j);
      int gray = basic_transform(px[0], px[1], px[2]);
      px[0] = gray;
      px[1] = gray;
      px[2] = gray;
      img.at<Vec3b>(i, j) = px;
...
void collect(Mat &img, int nrProcs) {
...
void master(Mat &img, int nrProcs) {
